...
def test_create_comment_with_image(client):
    pid = _create_post(client)
    r = client.post(
        f"/api/posts/{pid}/comments",
        headers={"X-Anon-Id": ANON_A},
...
def test_list_comments_sorted_desc_with_mine_flag(client):
    pid = _create_post(client)
    client.post(f"/api/posts/{pid}/comments", headers={"X-Anon-Id": ANON_A},
                data={"content": "first"})
    client.post(f"/api/posts/{pid}/comments", headers={"X-Anon-Id": ANON_B},
                data={"content": "second"})
    r = client.get(f"/api/posts/{pid}/comments", headers={"X-Anon-Id": ANON_A})
...
def test_delete_comment_only_by_author(client):
    pid = _create_post(client)
    cid = client.post(
        f"/api/posts/{pid}/comments",
        headers={"X-Anon-Id": ANON_A},
    ...
    r = client.delete(f"/api/comments/{cid}", headers={"X-Anon-Id": ANON_B})
    assert r.status_code == 403
    r2 = client.delete(f"/api/comments/{cid}", headers={"X-Anon-Id": ANON_A})
    assert r2.status_code == 204


def test_delete_comment_removes_image_file(client):
    pid = _create_post(client)
    body = client.post(
        f"/api/posts/{pid}/comments",
        headers={"X-Anon-Id": ANON_A},
    ...
    r = client.delete(f"/api/comments/{body['id']}", headers={"X-Anon-Id": ANON_A})
